...
<template>
    <div class="px-8 pt-8 bg-[#E2E8F0B3] shadow w-full rounded form-box">
        <form @submit.prevent="sendForm" class="w-full">
            <div class="grid grid-cols-12 gap-4">
                <div class="mb-3 text-left col-span-12 lg:col-span-6">
                    ...
                    <input type="text" class="rounded border-2 border-gray-200 focus:border-blue-400 transition-all duration-500 ease-in-out w-full" required id="name" v-model="form.name">
                </div>
                <div class="mb-3 text-left col-span-12 lg:col-span-6">
                    <label for="guest_email" class="form-label">{{ fieldsInLanguages[props.lang].email }}</label>
                    <input type="email" class="rounded border-2 border-gray-200 focus:border-blue-400 transition-all duration-500 ease-in-out w-full" required id="guest_email" v-model="form.guest_email">
                </div>
                <div class="mb-3 text-left col-span-12">
                    <label for="subject" class="form-label">{{ fieldsInLanguages[props.lang].subject }}</label>
                    <input type="text" class="rounded border-2 border-gray-200 focus:border-blue-400 transition-all duration-500 ease-in-out w-full" required id="subject" v-model="form.subject">
                </div>
                <div class="text-left col-span-12">
                    <label for="message" class="form-label">{{ fieldsInLanguages[props.lang].message }}</label>
                    <textarea class="rounded border-2 border-gray-200 focus:border-blue-400 transition-all duration-500 ease-in-out w-full" id="message" required rows="3" v-model="form.message"></textarea>
                </div>
